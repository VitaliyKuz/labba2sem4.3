...
namespace labba2sem4._3
{
    class Input
    {
        ...
        {
            StreamReader fromFile = new StreamReader(@"C:\Users\Laptop\Desktop\НУЛП\bases.txt");

            return fromFile.ReadToEnd().Split("|", StringSplitOptions.RemoveEmptyEntries);
        }
    }
}
...
namespace labba2sem4._3
{
    class Work
    {
        ...
        {
            StreamWriter save = new StreamWriter(@"C:\Users\Laptop\Desktop\НУЛП\bases.txt", true);
        ...
        public static void Parse(string[] elements, bool save)
        {
            ...
            for (int i = 0; i+4 < elements.Length; i +=5 )
            {
                Program.schedule[counter + i / 5] = new Schedule(Convert.ToInt16(Math.Floor(i / 5f)), elements[i + 1], elements[i + 2], elements[i + 3], elements[i + 4]);
                if (save)
                {
